...
if(type == "GEO"){
  ...
  if(!dir.exists(sradir)){
    print("Downloading data...")
    system(paste0('mkdir -p ', sradir))
    getSRAfile(in_acc = id, sra_con = con, fileType = 'sra', destDir = sradir, makeDirectory = TRUE, ascpCMD = 'ascp -QT -l 300m -i /usr/local/aspera/connect/etc/asperaweb_id_dsa.putty')
  }
}
